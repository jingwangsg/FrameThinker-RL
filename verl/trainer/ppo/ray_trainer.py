...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        # keeping the same workload as training
        bsz = self.config.data.train_batch_size * 2
        ...
        for test_data in tqdm(self.val_dataloader, desc="Validating"):
            ...
            if "raw_prompt" in test_batch.non_tensor_batch.keys():
                test_gen_batch.non_tensor_batch["raw_prompt"] = (
                    test_batch.non_tensor_batch.pop("raw_prompt")
                )

            if self.config.actor_rollout_ref.rollout.agent.activate_agent:
                tool_name_key = (
                    self.config.actor_rollout_ref.rollout.agent.tool_name_key
                )
                if (
                    tool_name_key
                    and tool_name_key in test_batch.non_tensor_batch.keys()
                ):
                    test_gen_batch.non_tensor_batch[tool_name_key] = (
                        test_batch.non_tensor_batch.pop(tool_name_key)
                    )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "raw_prompt" in batch.non_tensor_batch.keys():
                    gen_batch.non_tensor_batch["raw_prompt"] = (
                        batch.non_tensor_batch.pop("raw_prompt")
                    )
                    print(" [DEBUG raw prompt] raw_prompt pop into gen_batch")

                print(
                    f" [DEBUG config] config={self.config.actor_rollout_ref.rollout.agent}"
                )
                if self.config.actor_rollout_ref.rollout.agent.activate_agent:
                    tool_name_key = (
                        self.config.actor_rollout_ref.rollout.agent.tool_name_key
                    ...
                    if tool_name_key and tool_name_key in batch.non_tensor_batch.keys():
                        gen_batch.non_tensor_batch[tool_name_key] = (
                            batch.non_tensor_batch.pop(tool_name_key)
                        )
                ...
                with _timer("step", timing_raw):
                    ...
                    print("batch before old_log_prob")
                    batch.pretty_print()
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        try:
                            ...
                            reward_tensor, acc_tensor, format_tensor, other_tensor = (
                                self.reward_fn(batch)
                            )
                            reward_extra_infos_dict = {}

                        batch.batch["token_level_scores"] = reward_tensor
                        batch.batch["acc_tensor"] = acc_tensor
                        batch.batch["format_tensor"] = format_tensor
                        batch.batch["other_tensor"] = other_tensor
