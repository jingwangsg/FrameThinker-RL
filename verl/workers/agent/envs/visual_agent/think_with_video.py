...
class ThinkWithVideo(ToolBase):
    ...
    def execute(self, action_string, **kwargs):
        if not self.video_path:
            return "", 0.0, True, {}
        action_block = extract_tool_call_contents(
        ...
        if zoom_match:
            ...
            try:
                ...
                frame_tensor = self.vr_highres[frame_idx]  # [C, H, W]
                ...
                print(
                    f"[ERROR] Failed to zoom in on frame {frame_idx} from video '{self.video_path}': {e}"
                )
                return "", 0.0, True, {}
        ...
        obs_dict = {
            "prompt": all_user_msg,
            "multi_modal_data": {"image": focused_images_data},
        }
        return obs_dict, 0.0, False, {}
    ...
    def _process_zoom_request_no_desc(
        ...
        try:
                ...
                list(
                    set(
                        map(
                            int,
                            np.linspace(
                                sample_start, sample_end, self.num_frames_per_sample
                            ),
                        )
                    )
                )
            ...
            frame_batch = self.vr.get_frames_at(frame_indices)
            ...
            print(
                f"[ERROR] Failed to process video '{self.video_path}' between frames {start_frame}-{end_frame}."
            )
            print(
